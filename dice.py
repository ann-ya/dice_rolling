...
def parse_input(input_string):
    """
    Return 'input_string' as a integer between 1 and 6

    Check if 'input_string' is an integer number between 1 and 6.
    If so, return an integer with the same value.
    Otherwise, tell the user to enter valid input and quit the program.
    """
    if input_string.strip() in {"1", "2", "3", "4", "5", "6"}:
        return int(input_string)
    else:
        print("Please enter the number from 1 to 6.")
        raise SystemExit(1)


def roll_dice(num_dice):
    """
    Return a list of integers of lenght 'num-dice'.

    Each integer in a returned list is a random number between 1 and 6, inclusive.
    """
    roll_results = []
...
num_dice_input = input("How many dice do you want to roll? [0-6] ")
